...
export async function startBackgroundCli(logger: Logger, stateHome: string) {
  ...
  const existing = existsSync(stateHome)
    ? serviceUrl(await run(binary, ["service", "status"], logger, { stateHome }))
    : undefined
