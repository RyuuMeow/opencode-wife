...
- session prompts use the active agent plus Wife-specific model/variant with JSON-schema output (`messages` 1+, `choices` 2–3); OpenCode DeepSeek V4 models use an equivalent tagged text contract because their thinking mode rejects the required tool choice, and both live replies and restored history pass through the same reply validator
- assistant replies use as many short bubbles as needed to complete the thought; `<keep>...</keep>` preserves the rare multi-sentence unit that would sound unnatural when split, and bubbles reveal with language-aware reading delays based on CJK characters, words, and punctuation
- thinking uses a centered three-dot status animation with reduced-motion fallback; replies always request 2–3 model-authored follow-up choices, and a 2–3 item plain-text Markdown list is recovered as choices if a provider ignores the tagged contract
