...
- global `settings.v3` bubble controls configure height ratio, 13/15/17px text, fast/natural/relaxed reveal pacing, semantic background contrast, full/subtle/off motion, and character headers on every bubble/once per turn/hidden. Live and history bubbles share one presenter; reduced-motion always disables decorative movement.
